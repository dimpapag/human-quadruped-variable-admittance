...
int main(int argc, char** argv) {
    ...
    if (enable_logging) {
        ...
        std::string directory = "/home/con/catkin_ws/src/command_give/Admittance_Logging";
        struct stat info;
        ...
        info_log << "time(sec),v_x(m/s),v_y(m/s),omega_z(rad/s),f_x(N),f_y(N),tau_z(Nm),p_x(m),p_y(m),theta_z(rad)" << std::endl;
    }
    ...
    while (ros::ok()&& t<10.0) {
        ...
        if (enable_logging) {
            info_log << std::fixed << std::setprecision(6) << t << "," << vb(0) << "," << vb(1) << "," << wb << "," << Fext(0) << "," << Fext(1) << "," << Fext(2) << "," << pose(0) << "," << pose(1) << "," << pose(2) << std::endl;
        }
